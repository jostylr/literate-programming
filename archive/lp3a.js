...
var lineparser = function(lp) {
  ...
  var doc = {
    ...
    defaultProcessors: [function(line, doc) {
      var code = /^ {4}(.+)$/;
      ...
      var match, level, oldLevel, cur, name;
      var head = /^(\#+)\s*(.+)$/;
      ...
      if(match) {
        ...
        doc.level = level;
        doc.name = name;
        doc.pre[name] = function(code, doc) {
          return code;
        };
        doc.post[name] = function(code, doc) {
          return code;
        };
...
var oneSub = function oneSub(code, name, doc) {
    ...
    if (name === "File directive") {
      console.log(doc.blocks[name]);
    }
...
var fullSub = function(name, doc) {
    ...
    console.log(name, doc.blocks[name]);
